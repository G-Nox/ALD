...
public class ScotlandYard {
	...
	public static DirectedGraph<Integer> getGraph() throws FileNotFoundException {
		...
		Scanner in = new Scanner(new File("/home/g-nox/Insync/ald1/src/aufgabe3/ScotlandYard_Kanten.txt"));

		// ...
		while (in.hasNextLine()) {
			String[] s = in.nextLine().split("\\s+");
			Integer v = Integer.parseInt(s[0]);
			Integer w = Integer.parseInt(s[1]);
			String strWeight = s[2];
			double weight = 0;

			if (strWeight.equals("Taxi"))
				weight = 2.0;
			else if (strWeight.equals("Bus"))
				weight = 3.0;
			else if (strWeight.equals("UBahn"))
				weight = 5.0;
			else weight = 0.0;

			if (!sy_graph.containsEdge(v, w) || weight <= sy_graph.getWeight(v,w)) {
...
class ScotlandYardHeuristic implements Heuristic<Integer> {
	...
	public ScotlandYardHeuristic() throws FileNotFoundException {
		// ...
		coord = new TreeMap<>();
		Scanner in = new Scanner(new File("/home/g-nox/Insync/ald1/src/aufgabe3/scotlandYard/ScotlandYard_Knoten.txt"));
		while (in.hasNextLine()) {
